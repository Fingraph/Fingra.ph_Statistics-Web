...
function getFingraphData() {
    $.ajax({
        url: '<c:url value="/today/getPageViewsAjax" />',
        data: $('#fingraphSearchParam').serialize(),
        type: 'POST',
        dataType: 'json',
        success: function (data) {
            displayPageViewsFigures(data.figures); // 수치
            displayPageViewsLineGrap(data.list); // newUser Today line grap
            displayTimeSeriesPageViewsGrap(data.list, data.figures); // newUser Today timeSeries grap
            displayPageViewsExcelTable(data.list);//pageViews Today Excel
        }
    });
}
...
function displayPageViewsLineGrap(result){
	...
	var title = '<spring:message code="today.graph.title.cumul.pageview"/>';
	...
	makeDefaultLineChartToday(chart, 'container1', title, subTitle , '<spring:message code="chart.perf.pageView.y.text"/>', cate, xstep, data );
}

//newUser Today timeSeries grap
function displayTimeSeriesPageViewsGrap(result, figures){
	if( result == null) return;
	var chart = null;
	...
	if(distinctionTime != 0){
		var cate = new Array(24 + Math.abs(distinctionTime));
	}else{
		var cate = new Array(24);
	}
	var data = new Array(24);
	...
	var title = '<spring:message code="today.graph.title.pageview"/>';
	var subTitle = '(<spring:message code="today.card.today"/>)';
    if (moment($('#fingraphSearchParam #today').val()).diff(moment(),'days') !=0){
        subTitle = '(' + $('#fingraphSearchParam #today').val() + ')';
    }
	makeDefaultColumnChartToday(chart, 'container2', title, subTitle, '<spring:message code="chart.perf.pageView.y.text"/>', cate , data, total);
}

//pageViews Today Excel
	...
	<!-- graph start -->
		<div class="sub-content">
			<div class="sub-title-wrap">
				...
				<h2 class="title">
					<spring:message code="dash.card.pageView.title" />
				</h2>
			</div>
			<div class="box">
