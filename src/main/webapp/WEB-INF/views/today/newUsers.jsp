...
/* var timecheck = -(new Date().getTimezoneOffset()/60);
var koreaTime = 9;
var distinctionTime = timecheck - koreaTime;
var distinctionTime = 9; // test */

function getFingraphData(){
    $.ajax({
        url : '<c:url value="/today/getNewUsersAjax" />',
        data : $('#fingraphSearchParam').serialize(),
        type : 'POST',
        dataType : 'json',
        success : function(data){
            displayNewUsersFigures(data.figures); // 수치
            displayNewUsersLineGrap(data.list); // newUser Today line grap
            displayTimeSeriesNewUsersGrap(data.list, data.figures); // newUser Today timeSeries grap
            displayNewUsersExcelTable(data.list); // newUser Today Excel
        }
    });
}
$(function(){
...
// newUser Today line grap
function displayNewUsersLineGrap(result){
	...
	var title = '<spring:message code="today.graph.title.cumul.newUsers"/>';
	...
	makeDefaultLineChartToday(chart, 'container1', title, subTitle , '<spring:message code="chart.perfomence.newUser.y.text"/>', cate, xstep, data );
}

//newUser Today timeSeries grap
function displayTimeSeriesNewUsersGrap(result, figures){
	if( result == null) return;
	var chart = null;
	...
	var title = '<spring:message code="today.graph.title.newUsers"/>';
	var subTitle = '(<spring:message code="today.card.today"/>)';
    if (moment($('#fingraphSearchParam #today').val()).diff(moment(),'days') !=0){
        subTitle = '(' + $('#fingraphSearchParam #today').val() + ')';
    }
	makeDefaultColumnChartToday(chart, 'container2', title, subTitle, '<spring:message code="chart.perfomence.newUser.y.text"/>', cate , data, total);
}

// newUser Today Excel
function displayNewUsersExcelTable(result){
	// 헤더만 남기고 전부 삭제
	...
	<!-- graph start -->
		<div class="sub-content">
			<div class="sub-title-wrap">
				...
				<h2 class="title">
					<spring:message code="dash.card.newUser.title" />
				</h2>
			</div>
			<div class="box">
