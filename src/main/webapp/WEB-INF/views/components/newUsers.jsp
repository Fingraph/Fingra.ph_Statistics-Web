...
<meta name="currentMenu" content="newusers" />
...
function getFingraphData(){
	...
	$.ajax({
		url: '<c:url value="/components/getNewUsersAjax"/>',
		data: $('#fingraphSearchParam').serialize(),
		type: 'POST',
		dataType: 'json',
		success: function(data) {
			displayNewUsers(data.tlist,data.tslist);
			displayNewUsersTable(data.alist);
			displayNewUsersExcelTable(data.tlist,data.tslist);
		}
	});
}
//newUsers Excel table
function displayNewUsersExcelTable(tlist, tslist){
...
//newUser graph&table
function displayNewUsers(tlist, tslist){
	...
	var barTitle='<spring:message code="chart.comp.newUser.bar.title.topc"/>';
	var lineTitle=termText +'<spring:message code="chart.comp.newUser.line.title.topc"/>';
	if (groupVal==-1){
    ...
    makeDefaultColumnChart(chart, 'barChart',barTitle, subTitle, '<spring:message code="chart.comp.newUser.var.y.text"/>', category, column_data_array,total);

    if(tslist == null){
       	makeMultiLineChart(chart, 'line', lineTitle, subTitle, '<spring:message code="chart.comp.newUser.line.y.text"/>', '','', '');
       	return;
    }
		...
		makeMultiLineChart(chart, 'line', lineTitle, subTitle, '<spring:message code="chart.comp.newUser.line.y.text"/>', cate, xstep, line_data_array);

}
function displayNewUsersTable(result){
	// remove all except header
...
<body>
	...
	<jsp:include page="/components/include/section" flush="true">
	 ...
	 <jsp:param name="subMenu" value="newUsers" />
	</jsp:include>
	<!-- section end -->
	<!-- graph start -->
	<div class="sub-content">
	   	<div class="sub-title-wrap">
	   		...
	   		<h2 class="title"><spring:message code="dash.card.newUser.title"/></h2>
	       </div>
	       <div class="box">
                ...
                <table class="comp-table" id="actual_table">
                    ...
                    <thead>
	                    <tr>
	                        ...
	                        <th scope="col" class="top"><spring:message code="chart.comp.table.actualNu"/></th>
	                        <th scope="col" class="top"><spring:message code="chart.comp.table.totalNu"/></th>
	                        <th scope="col" class="top"><spring:message code="chart.comp.table.ofTotalNu"/></th>
