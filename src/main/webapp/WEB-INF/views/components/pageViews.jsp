...
function getFingraphData(){
	...
	$.ajax({
		url: '<c:url value="/components/getPageViewsAjax"/>',
		data: $('#fingraphSearchParam').serialize(),
		type: 'POST',
		dataType: 'json',
		success: function(data) {
			displayPageViews(data.tlist,data.tslist);
			displayPageViewsTable(data.alist);
			displayPageViewsExcelTable(data.tlist,data.tslist);
		}
	});
}

//pageviews Excel table
function displayPageViewsExcelTable(tlist, tslist){
...
function displayPageViews(tlist, tslist){
	var chart = null;
	var term = $('#term').val();
	...
	var barTitle='<spring:message code="chart.comp.pageViews.bar.title.topc"/>';
	var lineTitle=termText +'<spring:message code="chart.comp.pageViews.line.title.topc"/>';
	if (groupVal==-1){
    ...
    makeDefaultColumnChart(chart, 'barChart', barTitle, subTitle, '<spring:message code="chart.perf.pageView.y.text"/>', category, column_data_array,total);

    if(tslist == null){
       	makeMultiLineChart(chart, 'line', lineTitle, subTitle, '<spring:message code="chart.perf.pageView.y.text"/>', '','', '');
       	return;
    }
	...
	makeMultiLineChart(chart, 'line', lineTitle, subTitle, '<spring:message code="chart.perf.pageView.y.text"/>', cate, xstep, line_data_array);
}
function displayPageViewsTable(result){
	// remove all except header
                ...
                <table class="comp-table" id="actual_table">
                    ...
                    <thead>
	                    <tr>
	                        ...
	                        <th scope="col" class="top"><spring:message code="chart.comp.table.actualPv"/></th>
	                        <th scope="col" class="top"><spring:message code="chart.comp.table.totalPv"/></th>
	                        <th scope="col" class="top"><spring:message code="chart.comp.table.ofTotalPv"/></th>
