...
function getFingraphData(){
	$.ajax({
		url: '<c:url value="/performance/getActiveUsersAjax"/>',
		data: $('#fingraphSearchParam').serialize(),
		type: 'POST',
		dataType: 'json',
		success: function(data) {
			displayActiveUsersFigures(data.figures);
			displayActiveUsers(data.figures,data.list);
			displayActiveUsersExcelTable(data.list);
		}
	});
}
...
function displayActiveUsers(figures,result){
	var chart = null;
	var term = $('#term').val();
	...
	makeDefaultLineChart(chart, 'container', $("#term option:selected").text() +' <spring:message code="chart.perf.activeUser.y.text"/>', subTitle, '<spring:message code="chart.perf.activeUser.y.text"/>', cate, xstep, data, parseInt(figures.average));
}
</script>
</head>
<body>
