...
function getFingraphData(){
	$.ajax({
		url: '<c:url value="/performance/getNewUsersAjax"/>',
		data: $('#fingraphSearchParam').serialize(),
		type: 'POST',
		dataType: 'json',
		success: function(data) {
			displayNewUsersFigures(data.figures);
			displayNewUsers(data.figures,data.list);
			displayNewUsersExcelTable(data.list);
		}
	});
}
...
//newUser Daily graph&table
function displayNewUsers(figures,result){
	...
	for(var i = 0;i<result.length;i++) {
		var row = result[i];
		if(term == 'daily'){
			cate[i] = moment(row.date).format('MMM D, YYYY'); // x
		}else if(term=='weekly'){
	...
	makeDefaultLineChart(chart, 'container', $("#term option:selected").text() +' <spring:message code="chart.perfomence.newUser.y.text"/>', subTitle , '<spring:message code="chart.perfomence.newUser.y.text"/>', cate, xstep, data, parseInt(figures.average));
}
</script>
</head>
<body>
	...
	<div class="sub-content">
      ...
      <div class="figure-wrap">
          ...
          <colgroup>
              ...
              <tr>
                  <th scope="col"><spring:message code="chart.subcnt.title.totalUser"/></th>
